/**
 * event-hero — event/class-detail hero (navy wave + topo band).
 * Tier: TEMPLATE-SLOTTED. Holds the fixed composition (topo + wave SVGs,
 * breadcrumb + h1 + action row) and slots the authored breadcrumb, page <h1>,
 * and the hero CTAs. Uses the CSS-separator .ds-crumbs (no "/" text nodes).
 *
 * Source: stardust/prototypes/event-aquatic-aerobics-proposed.html (section.ds-hero)
 * Schema: stardust/eds-schema/event-aquatic-aerobics.json#event-hero
 *
 * Authored row contract:
 *   0  breadcrumb — one cell: the <a> crumbs + a trailing <span> current crumb
 *   1  <h1>…</h1>                              (the single page H1)
 *   2  CTAs — <em><strong><a></a></strong></em> (mint Register) + <em><a></a></em> (phone)
 */
...
export default async function decorate(block) {
  ...
  const [crumbRow, h1Row, ctaRow] = rows.map(cellOf);

  const crumbLinks = [...(crumbRow?.querySelectorAll('a') || [])];
  const currentText = (crumbRow?.querySelector('span')?.textContent || '').trim();
  const crumbLis = [
    ...crumbLinks.map((a) => `<li><a href="${a.getAttribute('href')}">${a.textContent.trim()}</a></li>`),
    currentText ? `<li><span aria-current="page">${currentText}</span></li>` : '',
  ].join('');
  ...
  copy.innerHTML = `
    <nav class="ds-crumbs" aria-label="Breadcrumb"><ol>${crumbLis}</ol></nav>
    <h1>${h1Text}</h1>`;
