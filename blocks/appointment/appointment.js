...
export default function decorate(block){
  ...
  panel.append(h);
  rows.forEach((r)=>{const t=(r.textContent||'').trim();if(t){const p=document.createElement('p');p.className='appointment-intro';p.innerHTML=r.firstElementChild?.innerHTML||t;panel.append(p);}});
  const fields=[['First name','text','fname'],['Last name','text','lname'],['Email','email','email'],['Phone','tel','phone'],['Preferred location','select','location'],['Reason for visit','text','reason']];
  ...
  fields.forEach(([label,type,name])=>{const w=document.createElement('div');w.className='appointment-field'+(name==='reason'?' appointment-field--wide':'');
    ...
    let ctl;if(type==='select'){ctl=document.createElement('select');['Select a location','Noblesville','Westfield'].forEach((o,i)=>{const op=document.createElement('option');op.textContent=o;if(i===0)op.disabled=true,op.selected=true;ctl.append(op);});}
